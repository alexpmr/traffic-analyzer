...
function renderHelp(){
  ...
  if(currentLang==='en'){
    el.innerHTML=`<h2>Traffic Analyzer Help</h2>
      ...
      <h3>2. Nodes</h3><p>The Nodes tab inventories every node known to the configured MeshMonitor source. Click any column heading to sort ascending or descending. Last interaction is color-coded: green up to 1 hour, yellow from more than 1 hour through 12 hours, and red above 12 hours. The Columns menu can reveal RSSI, channel utilization, Air Util TX, Node ID, PKC and state. Distance is calculated from the local node of the configured MeshMonitor source when both positions are known. Click a row to open the node details.</p>
      ...
      <h3>11. Interpretation limits</h3><div class="helpCallout helpWarn">Traffic Analyzer describes observed data. RF meshes are dynamic: absence of traffic does not by itself prove an outage; a traceroute is evidence of a route observed at a point in time; a relay byte does not always identify a complete path.</div>`;
  }else{
    el.innerHTML=`<h2>Ajuda do Traffic Analyzer</h2>
      ...
      <h3>2. Nós</h3><p>A aba Nós lista todos os nós conhecidos pela fonte MeshMonitor configurada. Clique em qualquer título de coluna para alternar entre ordem crescente e decrescente. A última interação usa cores: verde até 1 hora, amarelo acima de 1 hora até 12 horas e vermelho acima de 12 horas. O menu Colunas permite exibir RSSI, utilização do canal, Air Util TX, Node ID, PKC e estado. A distância é calculada a partir do nó local da fonte MeshMonitor quando as duas posições são conhecidas. Clique em uma linha para abrir os detalhes do nó.</p>
